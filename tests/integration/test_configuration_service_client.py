...
class TestConfigurationValue:
    ...
    def test_configuration_value_with_environment(self):
        ...
        config = ConfigurationValue(
            key="test_coordinator.chaos_config.timeout",
            value="60",
            type="number",
            environment="testing"
        )
    ...
    def test_configuration_value_validation_valid_types(self):
        ...
        valid_configs = [
            ConfigurationValue(key="app.name", value="test-coordinator", type="string"),
            ConfigurationValue(key="app.port", value="8083", type="number"),
    ...
    def test_configuration_value_type_conversions(self):
        ...
        number_config = ConfigurationValue(
            key="app.max_scenarios",
            value="50",
            type="number"
        )
        assert number_config.as_float() == 50.0
        assert number_config.as_int() == 50
...
class TestConfigurationServiceClient:
    ...
    async def test_configuration_caching(self, client):
        ...
        with patch('httpx.AsyncClient.get') as mock_get:
            ...
            mock_response.status_code = 200
            mock_response.json.return_value = {
                "key": "test_coordinator.cache_test",
                "value": "cached_value",
                "type": "string",
                "environment": "testing"
            }
            ...
            config1 = await client.get_configuration("test_coordinator.cache_test")
            assert mock_get.call_count == 1
            assert client.cache_hits == 0
            assert client._cache_misses == 1

            # Second call should hit the cache
            config2 = await client.get_configuration("test_coordinator.cache_test")
    ...
    async def test_cache_expiration(self, client):
        ...
        with patch.object(client, '_cache_ttl', 0.1):  # 100ms TTL
            with patch('httpx.AsyncClient.get') as mock_get:
                ...
                # First call
                config1 = await client.get_configuration("test_coordinator.ttl_test")
                ...
                config2 = await client.get_configuration("test_coordinator.ttl_test")
    ...
    async def test_concurrent_configuration_requests(self, client):
        ...
        with patch('httpx.AsyncClient.get') as mock_get:
            ...
            mock_response.status_code = 200
            mock_response.json.return_value = {
                "key": "test_coordinator.concurrent_test",
            ...
            tasks = [
                client.get_configuration("test_coordinator.concurrent_test")
                for _ in range(5)
            ]
    ...
    def test_configuration_error_types(self):
        ...
        assert str(error_with_key) == "Configuration error for key 'missing.key': Not found"
